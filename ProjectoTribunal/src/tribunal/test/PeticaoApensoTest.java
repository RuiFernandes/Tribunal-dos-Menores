...
/**
 * <p>
 * This test is generated for the entity 'PeticaoApenso' defined in the HEDL model.
 * </p>
 * 
 * <p>
 ...
 * </p>
 */
public class PeticaoApensoTest {
	...
	public void testCreate() {
		PeticaoApenso newEntity = createInstance(dao);
		assertNotNull("Newly created entity must not be null.", newEntity);
	}
	
	@Test
	public void testCount() {
		assertEquals("Empty database must contain no entities.", 0, dao.countPeticaoApensos(true));
	}
	...
	public void testCreateAndCount() {
		...
		PeticaoApenso newEntity = createInstance(dao);
		assertNotNull("Newly created entity must not be null.", newEntity);
		assertEquals("Database must contain 1 entity after creating one.", 1, dao.countPeticaoApensos(true));
	}
	...
	public void testDatePropertyData() {
		...
		assertEquals(1, dao.countPeticaoApensos(true));
		...
		newEntity = dao.getPeticaoApenso(id);
		assertEquals(newDate, newEntity.getData());
		
		List<PeticaoApenso> before = dao.getPeticaoApensosWithDataBefore(newDate);
		assertNotNull(before);
		assertEquals(0, before.size());
		
		List<PeticaoApenso> after = dao.getPeticaoApensosWithDataAfter(newDate);
		assertNotNull(after);
		...
		before = dao.getPeticaoApensosWithDataBefore(newDate);
		assertNotNull(before);
		assertEquals(0, before.size());
		
		after = dao.getPeticaoApensosWithDataAfter(newDate);
		assertNotNull(after);
	...
	public void testBooleanPropertyArchived() {
		...
		dao.setPeticaoApensosArchived(true, true);
		newEntity1 = dao.getPeticaoApenso(id1);
		assertTrue(newEntity1.isArchived());
	...
	private PeticaoApenso createInstance(TribunalDAO dao) {
